...
class HostRemover:
    ...
    def generate_host_removal_report(self, bowtie2_results: Dict[str, Any], 
                                   filter_results: Dict[str, Any]) -> str:
        """Generate a comprehensive host removal report."""
        report_file = "host_removal_report.html"
        
        html_content = self._create_host_removal_html_report(bowtie2_results, filter_results)
    ...
    def _create_host_removal_html_report(self, bowtie2_results: Dict[str, Any], 
                                       filter_results: Dict[str, Any]) -> str:
        """Create HTML host removal report."""
        html = """
        <!DOCTYPE html>
        <html>
        <head>
            <title>Host Removal Report</title>
            <style>
                body { font-family: Arial, sans-serif; margin: 20px; }
                .section { margin: 20px 0; padding: 10px; border: 1px solid #ddd; }
                .metric { margin: 10px 0; padding: 8px; background: #f9f9f9; }
        ...
        <body>
            <h1>Host Removal Report</h1>
        """
        
        # Add Bowtie2 results
        if bowtie2_results.get("success"):
            html += "<div class='section'><h2>Bowtie2 Alignment Results</h2>"
            for file, results in bowtie2_results.get("results", {}).items():
                html += f"<h3>{file}</h3>"
                if "statistics" in results:
                    stats = results["statistics"]
                    html += f"<div class='metric'><strong>Total reads:</strong> {stats.get('total_reads', 'N/A'):,}</div>"
                    html += f"<div class='metric'><strong>Aligned to host:</strong> {stats.get('aligned_reads', 'N/A'):,}</div>"
                    html += f"<div class='metric'><strong>Unaligned (kept):</strong> {stats.get('unaligned_reads', 'N/A'):,}</div>"
                    html += f"<div class='metric'><strong>Host alignment rate:</strong> {stats.get('alignment_rate', 'N/A'):.2f}%</div>"
            html += "</div>"
        
        # Add filtering results
        if filter_results.get("success"):
            html += "<div class='section'><h2>Host Removal Results</h2>"
            html += f"<p>Filtered files: {', '.join(filter_results.get('filtered_files', []))}</p>"
            html += "</div>"
