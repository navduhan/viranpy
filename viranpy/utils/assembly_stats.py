...
class AssemblyStatsCalculator:
    ...
    def run_quast_analysis(self, 
        ...
        try:
            ...
            results = {
                ...
                "report_file": self._generate_assembly_report(quast_results, viral_stats, output_dir)
            }
    ...
    def _generate_assembly_report(self, 
                                quast_results: Dict[str, Any],
                                viral_stats: Dict[str, Any],
                                output_dir: str) -> str:
        """Generate comprehensive assembly report."""
        # Always write to report_file inside self.config.root_output
        report_file = os.path.join(self.config.root_output, output_dir, "assembly_report.txt")
        
        with open(report_file, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("VIRAL METAGENOMIC ASSEMBLY STATISTICS REPORT\n")
            f.write("=" * 80 + "\n\n")
            
            # QUAST Results
            f.write("QUAST ASSEMBLY METRICS:\n")
            f.write("-" * 40 + "\n")
            for metric, value in quast_results.items():
                if metric != "contigs_detailed":
                    f.write(f"{metric}: {value}\n")
            f.write("\n")
            
            # Viral-specific statistics
            f.write("VIRAL-SPECIFIC STATISTICS:\n")
            f.write("-" * 40 + "\n")
            f.write(f"Total contigs: {viral_stats.get('total_contigs', 0):,}\n")
            f.write(f"Total bases: {viral_stats.get('total_bases', 0):,}\n")
            f.write(f"Largest contig: {viral_stats.get('largest_contig', 0):,} bp\n")
            f.write(f"Smallest contig: {viral_stats.get('smallest_contig', 0):,} bp\n")
            f.write(f"Mean contig length: {viral_stats.get('mean_contig_length', 0):.1f} bp\n")
            f.write(f"Median contig length: {viral_stats.get('median_contig_length', 0):,} bp\n")
            f.write(f"N50: {viral_stats.get('n50', 0):,} bp\n")
            f.write(f"N90: {viral_stats.get('n90', 0):,} bp\n")
            f.write(f"Mean GC content: {viral_stats.get('mean_gc_content', 0):.2f}%\n")
            f.write("\n")
            
            # Contig length distribution
            if viral_stats.get('contig_lengths'):
                lengths = viral_stats['contig_lengths']
                f.write("CONTIG LENGTH DISTRIBUTION:\n")
                f.write("-" * 40 + "\n")
                f.write(f"Contigs > 1kb: {len([l for l in lengths if l >= 1000])}\n")
                f.write(f"Contigs > 5kb: {len([l for l in lengths if l >= 5000])}\n")
                f.write(f"Contigs > 10kb: {len([l for l in lengths if l >= 10000])}\n")
                f.write(f"Contigs > 50kb: {len([l for l in lengths if l >= 50000])}\n")
                f.write("\n")
            ...
            if total_contigs > 0:
                f.write(f"Assembly completeness: {'Good' if total_bases >= 1000000 else 'Limited'}\n")
                f.write(f"Contig fragmentation: {'Low' if total_contigs <= 100 else 'High'}\n")
                f.write(f"Contig quality: {'Good' if n50 >= 5000 else 'Poor'}\n")
            
            f.write("\n" + "=" * 80 + "\n")
            f.write("Report generated by ViRAnPy Viral Metagenomic Pipeline\n")
            f.write("=" * 80 + "\n")
